...
module M2yFast
  class XmlBuilder
    ...
    def self.block_card_xml(card_id, trace, cod_input, close_account, virtual)
      "<Envelope xmlns='http://schemas.xmlsoap.org/soap/envelope/'>
        <Body>
            <bloqueio_cartao xmlns='http://WSGServ/'>
                ...
                <proxy xmlns=''>#{card_id}</proxy>
                <cartao xmlns=''>#{card_id}</cartao>
                <status xmlns=''>#{virtual ? ADMIN_BLOCKED_STATUS : close_account ? BLOCKED_STATUS_CANCEL_ACCOUNT : BLOCKED_STATUS}</status>
                <usuario xmlns=''>#{DEFAULT_USER}</usuario>
                <usr xmlns=''>#{M2yFast.configuration.username}</usr>
                <pwd xmlns=''>#{M2yFast.configuration.password}</pwd>
            </bloqueio_cartao>
